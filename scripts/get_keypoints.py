...
def run_mediapipe(video_path: str) -> np.ndarray:
    ...
    keypoints = extract(frames)

    # Calcula la relación de aspecto inverso del video (height / width)
    # Asumiendo que todas las frames tienen la misma dimensión
    height, width, _ = frames[0].shape
    inverse_aspect_ratio = height / width

    return postprocess(keypoints, inverse_aspect_ratio)
...
def main(str_path, path_npy):
    #out_dir = '/scratch/uduran005/tfg-workspace/keypoints/LIL' ##Antes LIL
    out_dir = path_npy
    path = os.listdir(str_path)
    path.sort()

    for selected_video in path:
        clip = str_path + selected_video
        video_name, _ = os.path.splitext(os.path.basename(clip))
        print(f"Video name --> {video_name}")
        output_path = os.path.join(out_dir, video_name + '.npy')        ##Antes era .npy
    
        if not os.path.isfile(output_path):
            keypoints = run_mediapipe(clip)
